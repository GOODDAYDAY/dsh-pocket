...
| 🔐 访问密码 | 公网链接需输入**访问密码**（自动生成 8 位含大小写字母与数字；默认每次开启公网自动换新；**可自定义固定密码**——自定义后不再换新）；局域网有独立**访问密码**（同规则，默认开启，设置页可**一键关闭**——关闭后局域网扫码直连） |
| 🔑 自定义密码 | 公网/局域网密码都可在设置页**设成自己固定的密码**：长度 **>6 位且 <128 位**，数字/大小写/特殊符号均可（自定义后公网不再自动换新） |
...
设置 → **手机访问** → 手机扫「📶 局域网」二维码 → 打开链接**输入局域网密码**（显示在设置页局域网区块，点「刷新」可换新，或点「自定义」设成自己固定的密码——7-127 位任意字符）→ 打开的就是电脑上的 DSH，实时同步。
...
同一页点「**开启公网访问**」→ **每次都会先弹出安全免责声明**，勾选「我已知情」后才能开启（公司/涉密网络请先确认合规）→ 等隧道建立（首次会下载 cloudflared，macOS/Linux 走清华镜像秒下）→ 手机扫「🌐 公网」二维码 → 打开链接**输入访问密码**（密码显示在设置页公网区块，默认**每次开启公网变新**，也可点「自定义」设成固定密码——7-127 位任意字符，自定义后不再换新）→ 人在外面（4G/公司网）也能访问。

> 更新到新版本：`dsh plugin --profile web update dsh-pocket --latest -w`（跨大版本时 `--latest` 是必须的，`^0.x` 范围不会自动升到 1.x）。

## ⚠️ 安全（必读）

- **DSH 能执行你电脑上的代码**。**局域网**二维码/URL 配上独立**访问密码**才是钥匙（密码**默认开启**，可关——关闭后局域网扫码直连，仅同一网络设备可访问），**请勿把局域网二维码、URL 或密码发给别人**
- **开启公网访问前必须阅读并勾选免责声明**（每次开启都会弹框；服务端强制校验，无法绕过）：公网 = 把能执行代码的 DSH 暴露到互联网，请使用强密码、用完即关、涉密网络勿用
- **公网**有**访问密码**保护（自动生成 8 位含大小写字母与数字；可自定义 7-127 位任意字符）：链接随机分配、默认每次开启换新密码、旧链接立即作废——泄露了也进不来，改密码/重开即可作废；**自定义密码后不再自动换新**（你设的值即稳定密码）
...
| 文件 | 说明 |
|---|---|
| `lib/index.js` | 插件入口：自动起代理 + 注册 RPC + 访问密码管理（自动生成 8 位含大小写字母+数字，公网每次开启变新；局域网独立密码可手动刷新/开关；自定义密码 7-127 位任意字符）+ 局域网访问总开关 + 桌面端环境适配 |
