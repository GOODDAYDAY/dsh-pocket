  ...
  return ctx.connection.rpc.handle(POCKET_RPC_CHANNEL, async (endpoint, payload = {}, signal) => {
    ...
    const statusPayload = async () => {
      ...
      return ok({
        ...
        macWhitelistEnabled: getMacWhitelistEnabled?.() ?? false,
        macWhitelist: getMacWhitelist?.() ?? [],
      });
    };
    ...
    try {
      ...
      if (endpoint === POCKET_ENDPOINTS.macWhitelistSetEnabled) {
        const enabled = setMacWhitelistEnabled?.(payload?.on === true);
        if (enabled === undefined) return fail('bad-request', 'MAC 白名单开关不可用 | MAC whitelist switch unavailable');
        return ok({ macWhitelistEnabled: enabled });
      }
      if (endpoint === POCKET_ENDPOINTS.macWhitelistSet) {
        // entries: [{ mac, note }]——全量替换（前端维护当前列表，增删/改备注都发全量）
        try {
          const macs = setMacWhitelist?.(payload?.entries);
          if (macs === undefined) return fail('bad-request', 'MAC 白名单设置不可用 | MAC whitelist setting unavailable');
          return ok({ macWhitelist: macs });
        } catch (err) {
          return fail('bad-request', err?.message ?? String(err));
        }
      }
      if (endpoint === POCKET_ENDPOINTS.macWhitelistDiscover) {
        // 设备发现：本机 ARP 表（局域网可见设备）。失败返回空列表，不让设置页报错。
        try {
