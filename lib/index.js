...
import { desktopEnvPatchScript, advancedNoticeScript, DEFAULT_INJECT, clientIp } from './proxy.mjs';
import { lanEnabled, setLanEnabled, lanAuthEnabled, setLanAuthEnabled, lanIpOverride, setLanIpOverride, pinCustom, setPinCustom, macWhitelistEnabled, setMacWhitelistEnabled, macWhitelist, setMacWhitelist } from './settings.mjs';
import { createArpResolver, checkMacWhitelist } from './arp.mjs';
...
// ---------- 访问密码（issue #13 + #18 + #33） ----------
// 公网与局域网**分开**，存本机 $DSH_HOME/dsh-pocket/。
// 自动生成的密码为 **8 位（A-Za-z0-9，含大小写字母）**，用 crypto.randomInt 保证不可预测；
// 自定义密码长度须 **> 6 位且 < 128 位**，字符集不限（数字/大小写/特殊符号均可）。
...
const PIN_AUTO_LEN = 8;
const PIN_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
/** 密码长度规则：> 6 位且 < 128 位，字符集不限。readPin 与 setCustomPin 共用。 */
function pinValid(v) {
...
function readPin(p) {
  try {
    ...
    if (pinValid(existing)) return existing;
  } catch { /* 无文件 */ }
  return null;
}
function newPin() {
  let s = '';
  for (let i = 0; i < PIN_AUTO_LEN; i++) s += PIN_ALPHABET[randomInt(PIN_ALPHABET.length)];
  return s;
}
...
/**
 * 用户自定义访问密码（issue #33）：公网/局域网各自设一个固定密码。
 * 自定义后公网开启时不再自动轮换（rotateAccessToken 见上）。
 * 非法输入（长度 ≤6 或 ≥128 位）抛错，由 RPC 层转成错误响应。
 */
export function setCustomPin(which, value) {
  const v = String(value ?? '').trim();
  if (!pinValid(v)) throw new Error('密码长度需大于 6 位且小于 128 位（数字、大小写、特殊符号均可）| PIN must be longer than 6 and shorter than 128 characters (digits, letters, and symbols are all allowed)');
  if (which === 'public') {
...
/**
 * 设备发现合并（纯函数）：把代理连接记录（含公网，mac 可为 null）与 ARP 表
 * （局域网，必有 mac）合并成设备列表，按最近连接降序。见 apply() 的 discoverDevices。
 */
export function mergeDevices(arpDevices, seenClients) {
  const byIp = new Map(arpDevices.map((d) => [d.ip, d]));
  const out = new Map();
  for (const s of seenClients) {
    const arp = byIp.get(s.ip);
    if (arp) out.set(s.ip, { ip: s.ip, mac: arp.mac, via: 'lan', lastSeen: s.lastSeen });
    else out.set(s.ip, { ip: s.ip, mac: null, via: s.via, lastSeen: s.lastSeen });
  }
  for (const d of arpDevices) {
    if (!out.has(d.ip)) out.set(d.ip, { ip: d.ip, mac: d.mac, via: 'lan', lastSeen: null });
  }
  return [...out.values()].sort((a, b) => (b.lastSeen ?? 0) - (a.lastSeen ?? 0));
}
...
export function apply(ctx, config = {}, internals = {}) {
  ...
  // 局域网 MAC 白名单（免密直连）：ARP 反查 IP → MAC（仅同子网可见，见 arp.mjs）。
  // resolver 进程级单例：缓存 IP→MAC 映射 30 秒；判定 fail-closed——缓存未命中先走
  // PIN（回退现有流程），后台预热 ARP 表，下次请求命中即免密。
  const arp = createArpResolver();
  ...
  const service = internals.service ?? createPocketService({
    ...
    auth: {
      ...
      // MAC 白名单（免密直连）：名单内设备跳过密码流程。fail-closed——缓存未命中
      // 先走 PIN 并后台预热（checkMacWhitelist 内部处理），下次请求命中即免密。
      // 仅局域网生效：公网隧道流量来自云端，ARP 表里查不到（见 arp.mjs）。
      isMacAllowed: (req) => checkMacWhitelist({
        enabled: macWhitelistEnabled(),
        macs: macWhitelist().map((e) => e.mac),
        ip: clientIp(req),
        peek: (ip) => arp.peek(ip),
        resolve: (ip) => arp.resolve(ip),
      }),
    },
  ...
  async function discoverDevices() {
    const [arpDevices, seenClients] = await Promise.all([
      arp.listAll(),
      Promise.resolve(proxyRef?.getSeenClients() ?? []),
    ]);
    return mergeDevices(arpDevices, seenClients);
  }
  ...
  const disposeRpc = installPocketRpc(ctx, {
    ...
    getMacWhitelistEnabled: () => macWhitelistEnabled(),
    setMacWhitelistEnabled: (on) => setMacWhitelistEnabled(on),
    getMacWhitelist: () => macWhitelist(),
    setMacWhitelist: (macs) => setMacWhitelist(macs),
    discoverDevices,
