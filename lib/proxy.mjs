...
input{width:100%;box-sizing:border-box;padding:10px 12px;font-size:16px;letter-spacing:1px;text-align:center;border:1px solid #d1d5db;border-radius:8px;outline:none;margin-bottom:12px}
...
<input name="token" type="password" maxlength="127" autocomplete="off" autofocus required>
 ...
 * @param {object} [opts.auth]       可选访问令牌认证（issue #13）：{ getToken, isProtected, sessionKey, isMacAllowed }
 *                                  isMacAllowed(req)：可选 MAC 白名单判定（局域网免密，issue/PR），同步返回布尔；命中 → 跳过密码流程
 ...
 * @param {boolean} [opts.trackClients] 记录最近连接过的客户端 IP（设备发现用，默认开）
 * @param {number} [opts.maxClients] 连接记录容量上限（默认 200，超出顶掉最旧）
 * @returns {Promise<{server:import('node:http').Server, close:()=>Promise<void>, getSeenClients:()=>Array}>}
 */
export function createPocketProxy({ port = 3081, host = '0.0.0.0', upstream = DEFAULT_UPSTREAM, log = null, injectHtml = DEFAULT_INJECT, auth = null, rateLimit = null, heartbeat = {}, trackClients = true, maxClients = 200 } = {}) {
  const limiter = auth ? createRateLimiter(rateLimit ?? {}) : null;
  // 最近连接过的客户端（设备发现来源之一，见 lib/index.js 的 discoverDevices）：
  // ip → { via: 'lan'|'public', lastSeen }。仅内存态——dsh web 重启后清空，
  // 但 ARP 表（内核缓存）仍在，局域网设备不会因此消失。容量上限防内存膨胀。
  const seen = new Map();
  const recordSeen = (req) => {
    if (!trackClients) return;
    const ip = clientIp(req);
    if (!ip || ip === 'unknown') return;
    const host = String(req.headers.host ?? '');
    seen.set(ip, { via: /trycloudflare\.com$/i.test(host) ? 'public' : 'lan', lastSeen: Date.now() });
    if (seen.size > maxClients) seen.delete(seen.keys().next().value); // 顶掉最旧
  };
  const server = createServer((req, res) => {
    ...
    if (auth) {
      ...
      // MAC 白名单（局域网免密直连）：仅 LAN 生效（公网拿不到 MAC），命中 → 跳过密码流程
      const macAllowed = !isPublic && auth.isMacAllowed?.(req) === true;
      if (!macAllowed && protectedHost && token) {
        const ip = clientIp(req);
  ...
  server.on('upgrade', (req, socket, head) => {
    ...
    if (auth) {
      const host = String(req.headers.host ?? '');
      // MAC 白名单（局域网免密直连）：与 HTTP 路径同规则——仅 LAN 生效，命中放行
      const macAllowed = !/trycloudflare\.com$/i.test(host) && auth.isMacAllowed?.(req) === true;
      const token = isProtectedHost(host, auth.isProtected) ? (auth.getToken?.(host) ?? null) : null;
      if (!macAllowed && token && !authCheck(req, token, auth.sessionKey ?? null)) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(port, host, () => {
      ...
      resolve({
        ...
        /** 最近连接过的客户端（最近优先）：[{ ip, via: 'lan'|'public', lastSeen }]（设备发现用）。 */
        getSeenClients: () => [...seen.entries()]
          .map(([ip, r]) => ({ ip, via: r.via, lastSeen: r.lastSeen }))
